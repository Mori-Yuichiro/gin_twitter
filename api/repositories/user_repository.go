...
func (ur *userRespository) GetUserByUserId(user *models.User, userId uint) error {
	...
	}).Preload("Followers.Follower").Preload("Followers.Followed").Preload("Followeds", func(db *gorm.DB) *gorm.DB {
		return db.Where("follower_id=?", userId)
	}).Preload("Followeds.Follower").Preload("Followeds.Followed").Where("id=?", userId).First(user).Error; err != nil {
		return err
	}
	return nil
}
